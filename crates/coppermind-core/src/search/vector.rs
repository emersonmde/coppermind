...
use super::types::{validate_dimension, ChunkId, SearchError};
...
pub struct VectorSearchEngine {
    ...
    /// Map from HNSW index position to ChunkId
...
impl VectorSearchEngine {
    ...
    #[instrument(skip_all, fields(index_size = self.chunk_ids.len()))]
    pub fn add_chunk(&mut self, chunk_id: ChunkId, embedding: Vec<f32>) -> Result<(), SearchError> {
    ...
    /// Add a chunk embedding without rebuilding the index
    ///
    /// With rust-cv/hnsw, this is identical to add_chunk() since incremental
    ...
    /// Returns `SearchError::DimensionMismatch` if embedding dimension doesn't match
    /// the engine's configured dimension.
    pub fn add_chunk_deferred(
        &mut self,
        chunk_id: ChunkId,
        embedding: Vec<f32>,
    ) -> Result<(), SearchError> {
        // rust-cv/hnsw supports incremental insertion, so this is the same as add_chunk
        self.add_chunk(chunk_id, embedding)
    }
    ...
    ///
    /// # Returns
    /// Vector of (ChunkId, similarity_score) pairs, sorted by similarity (descending)
    ...
    pub fn search(
        ...
        // Convert results to (ChunkId, score) pairs, filtering out tombstoned entries
        let results = neighbors
            ...
            .map(|neighbor| {
                ...
                let chunk_id = self.chunk_ids[neighbor.index];
                (chunk_id, similarity)
            })
            .collect();
        Ok(results)
    ...
    /// Get the ChunkId at a given index position.
    ///
    /// Returns None if the index is out of bounds.
    #[allow(dead_code)] // Public API
    pub fn get_chunk_id_at(&self, idx: usize) -> Option<ChunkId> {
        self.chunk_ids.get(idx).copied()
    }

    /// Find the index position for a ChunkId.
    ///
    /// Returns None if the ChunkId is not in the index.
    /// Note: This is O(n) linear search - use sparingly.
    #[allow(dead_code)] // Will be used in Phase 3 for tombstone persistence
    pub fn find_index(&self, chunk_id: ChunkId) -> Option<usize> {
    ...
    pub fn tombstone_ratio(&self) -> f32 {
        ...
        self.tombstones.len() as f32 / self.chunk_ids.len() as f32
    }
    ...
    pub fn live_count(&self) -> usize {
        self.chunk_ids.len() - self.tombstones.len()
    }

    /// Get all live (non-tombstoned) ChunkIds with their index positions.
    ///
    /// Used during compaction to know which embeddings to reload.
    pub fn get_live_entries(&self) -> Vec<(usize, ChunkId)> {
        self.chunk_ids
            .iter()
            .enumerate()
            .filter(|(idx, _)| !self.tombstones.contains(idx))
            .map(|(idx, &chunk_id)| (idx, chunk_id))
            .collect()
    }

    /// Rebuild the index from a list of (ChunkId, embedding) pairs.
    ///
    /// This creates a fresh HNSW index with only the provided entries,
    /// effectively compacting away all tombstoned entries.
    ///
    /// # Arguments
    /// * `entries` - Vec of (ChunkId, embedding) pairs to include in new index
    ///
    /// # Returns
    ...
    #[instrument(skip_all, fields(old_size = self.chunk_ids.len(), new_size = entries.len()))]
    pub fn compact(&mut self, entries: Vec<(ChunkId, Vec<f32>)>) -> Result<usize, SearchError> {
        ...
        let mut new_chunk_ids = Vec::with_capacity(entries.len());
        ...
        let compacted_count = new_chunk_ids.len();
...
mod tests {
    ...
    fn test_vector_search() {
        ...
        engine.add_chunk(chunk1, vec![1.0, 0.0, 0.0]).unwrap();
        engine.add_chunk(chunk2, vec![0.0, 1.0, 0.0]).unwrap();
        engine.add_chunk(chunk3, vec![1.0, 0.1, 0.0]).unwrap();

        // Search for something close to chunk1 and chunk3
        let results = engine.search(&[1.0, 0.0, 0.0], 2).unwrap();

        assert_eq!(results.len(), 2);
        assert_eq!(results[0].0, chunk1); // Exact match should be first
        assert_eq!(results[1].0, chunk3); // Similar vector should be second
    }

    #[test]
    fn test_add_chunk_updates_count() {
        let mut engine = VectorSearchEngine::new(3);
    ...
    fn test_incremental_insertion() {
        ...
        // Add chunks incrementally (no rebuild needed with rust-cv/hnsw)
        for i in 0..5 {
            let chunk_id = ChunkId::from_u64(i);
            engine
                .add_chunk(chunk_id, vec![i as f32, 0.0, 0.0])
                .unwrap();
        }

        // Index should have all chunks
    ...
    fn test_deferred_add_is_same_as_regular() {
        ...
        for i in 0..5 {
            let chunk_id = ChunkId::from_u64(i);
            engine
                .add_chunk_deferred(chunk_id, vec![i as f32, 0.0, 0.0])
                .unwrap();
        }

        // Chunks should be searchable immediately
        assert_eq!(engine.len(), 5);
        let results = engine.search(&[2.0, 0.0, 0.0], 3).unwrap();
        assert!(!results.is_empty());
    }
    ...
    fn test_exact_match_returns_high_similarity() {
        ...
        engine.add_chunk(chunk1, embedding.clone()).unwrap();
    ...
    #[test]
    fn test_add_chunk_dimension_mismatch() {
        let mut engine = VectorSearchEngine::new(3);
        let chunk_id = ChunkId::from_u64(1);

        // Try to add 2D embedding to 3D engine
        let result = engine.add_chunk(chunk_id, vec![1.0, 0.0]);
        assert!(matches!(
            result,
    ...
    fn test_search_dimension_mismatch() {
        let mut engine = VectorSearchEngine::new(3);
        let chunk_id = ChunkId::from_u64(1);
        engine.add_chunk(chunk_id, vec![1.0, 0.0, 0.0]).unwrap();
    ...
    fn test_tombstone_filters_results() {
        ...
        engine.add_chunk(chunk1, vec![1.0, 0.0, 0.0]).unwrap();
        engine.add_chunk(chunk2, vec![0.9, 0.1, 0.0]).unwrap();
        engine.add_chunk(chunk3, vec![0.0, 1.0, 0.0]).unwrap();

        // Search should return all 3 chunks
        let results = engine.search(&[1.0, 0.0, 0.0], 10).unwrap();
        assert_eq!(results.len(), 3);

        // Tombstone chunk1 (index 0)
        engine.mark_tombstone(0);

        // Search should now exclude chunk1
        let results = engine.search(&[1.0, 0.0, 0.0], 10).unwrap();
        assert_eq!(results.len(), 2);
    ...
    fn test_tombstone_persistence() {
        ...
        engine
            .add_chunk(ChunkId::from_u64(1), vec![1.0, 0.0, 0.0])
            .unwrap();
        engine
            .add_chunk(ChunkId::from_u64(2), vec![0.0, 1.0, 0.0])
            .unwrap();
    ...
    fn test_find_index() {
        ...
        let chunk1 = ChunkId::from_u64(100);
        let chunk2 = ChunkId::from_u64(200);
        let chunk3 = ChunkId::from_u64(300);

        engine.add_chunk(chunk1, vec![1.0, 0.0, 0.0]).unwrap();
        engine.add_chunk(chunk2, vec![0.0, 1.0, 0.0]).unwrap();
        engine.add_chunk(chunk3, vec![0.0, 0.0, 1.0]).unwrap();

        // Find indices
        assert_eq!(engine.find_index(chunk1), Some(0));
        assert_eq!(engine.find_index(chunk2), Some(1));
        assert_eq!(engine.find_index(chunk3), Some(2));

        // Non-existent chunk
        assert_eq!(engine.find_index(ChunkId::from_u64(999)), None);
    }
    ...
    fn test_tombstone_ratio() {
        ...
        for i in 0..10 {
            engine
                .add_chunk(ChunkId::from_u64(i), vec![i as f32, 0.0, 0.0])
                .unwrap();
        }
    ...
    fn test_live_count_and_entries() {
        ...
        assert!(live_entries.iter().any(|(_, id)| *id == chunk1));
        assert!(!live_entries.iter().any(|(_, id)| *id == chunk2));
        assert!(live_entries.iter().any(|(_, id)| *id == chunk3));
    }
    ...
    fn test_compact() {
        ...
        engine.add_chunk(chunk1, emb1.clone()).unwrap();
        engine.add_chunk(chunk2, emb2.clone()).unwrap();
        engine.add_chunk(chunk3, emb3.clone()).unwrap();
        ...
        let entries = vec![(chunk1, emb1.clone()), (chunk3, emb3.clone())];
        ...
        assert_eq!(results.len(), 2);
        assert!(results.iter().any(|(id, _)| *id == chunk1));
        assert!(results.iter().any(|(id, _)| *id == chunk3));
        assert!(!results.iter().any(|(id, _)| *id == chunk2)); // chunk2 was removed
    }
    ...
    fn test_compact_empty() {
        ...
        // Add some chunks and tombstone all of them
        engine
            .add_chunk(ChunkId::from_u64(1), vec![1.0, 0.0, 0.0])
            .unwrap();
        engine
    ...
    fn test_compact_dimension_mismatch() {
        ...
        let result = engine.compact(vec![(ChunkId::from_u64(1), vec![1.0, 0.0])]); // 2D instead of 3D
