...
use crate::embedding::config::JinaBertConfig;
use crate::embedding::model::{Embedder, JinaBertEmbedder};
...
impl SchedulerStatsInner {
    fn new() -> Self {
        Self {
            device_type: RwLock::new(DeviceType::default()),
...
impl SerialScheduler {
    ...
    fn worker_loop(
        ...
        // Store device type in stats for monitoring
            ...
            }) = queue.pop()
            {
                ...
                match message {
                    ...
                    SchedulerMessage::UnloadModel { model_id, response } => {
                        info!("🗑️ Unloading model: {}", model_id);
    ...
    fn do_load_model(
        ...
        match &model_id {
            ModelId::JinaBert => {
                ...
                info!(
                    "⚙️  JinaBERT config: {}d hidden, {} layers, {} heads",
...
fn create_jina_bert_model(
    ...
    use crate::embedding::config::ModelConfig;
    ...
    // Note: We need to access internal fields, so we'll construct directly
